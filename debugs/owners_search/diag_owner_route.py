...
def main() -> None:
    ...
    exact_body = {
        "query": {"term": {"name.keyword": {"value": args.query}}},
        "_source": ["mid", "name"],
        "size": 5,
    }
    ...
    payload = {
        "index": args.index,
        "query": args.query,
        "exact_hit": searcher._has_exact_name_hit(args.query),
        "query_route": searcher._detect_query_route(args.query),
        "exact_total": ((exact_raw.get("hits") or {}).get("total") or {}).get("value"),
