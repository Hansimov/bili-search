...
PROMPT_ASSETS: list[PromptAsset] = [
    ...
    _asset(
        ...
        "ROUTE_RELATIONS",
        "brief",
        "目标是关联账号、矩阵号、类似作者时，优先 search_owners(mode=relation) 或关系类工具，不要机械转成视频搜索。",
    ...
    _asset(
        ...
        "ROUTE_EXTERNAL",
        "brief",
        "目标是官网、公告、release notes、跨站事实时，优先 search_google。若产品名、版本号、作者名已经明确，不要先 related_tokens_by_tokens。若最终仍要回到 B 站内容，可把 Google 当侦察层，再继续视频或作者工具。",
        tags=("route", "external"),
    ),
    _asset(
        "route.mixed.brief",
        ...
        "ROUTE_MIXED",
        "brief",
        "mixed 任务要拆子目标分别完成，例如官方更新和 B 站解读、作者关系和代表作，不要把不同目标混成一轮模糊搜索。若产品名、版本号或主题实体已经明确，直接进入 search_google / search_videos，不要先做 related_tokens_by_tokens。通常先各执行一轮终局工具，再基于现有结果回答；不要反复同时重跑 search_google 和 search_videos。",
        tags=("route", "mixed"),
    ),
    _asset(
    ...
    _asset(
        ...
        "TOOL_SEARCH_VIDEOS",
        "detailed",
        "构造 search_videos 时，优先并行多条 queries 覆盖不同搜索假设。能稳定用 :user / :uid 时再定向；作者名不稳时先 search_owners。抽象需求先 related_tokens_by_tokens 或 search_google 侦察，再回到 search_videos 终局。",
        tags=("tool",),
        tool_name="search_videos",
    ),
    _asset(
    ...
    _asset(
        ...
        "related_tokens_by_tokens examples",
        "TOOL_RELATED_TOKENS",
        "examples",
