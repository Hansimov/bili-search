...
def test_execute_expand_query_defaults_to_auto_mode():
    logger.note("=" * 60)
    logger.note("[TEST] execute expand_query defaults to auto")
    ...
    logger.success("[PASS] execute expand_query defaults to auto")
...
def test_execute_expand_query_coerces_semantic_mode_to_auto():
    logger.note("=" * 60)
    logger.note("[TEST] execute expand_query coerces semantic to auto")
    ...
    tc = ToolCall(
        ...
        name="expand_query",
        arguments=json.dumps({"text": "袁启 专访", "mode": "semantic"}),
    )
    ...
    logger.success("[PASS] execute expand_query coerces semantic to auto")
